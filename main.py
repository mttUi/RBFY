...
def start(message):
    ...
    sent_message = bot.send_message(message.chat.id, "Здравствуйте!\n\nМеня зовут RBFU, и я здесь, чтобы помочь вам с расписанием обучения. Независимо от того, в какой группе вы находитесь, я предоставлю вам всю необходимую информацию о ваших занятиях, включая время и предметы.\nЧтобы узнать ваше расписание, просто укажите название вашей группы, и я предоставлю вам актуальную информацию. Если у вас возникнут какие-либо вопросы или нужна дополнительная помощь, не стесняйтесь задавать их.\n\nДавайте начнем! Какую группу вы представляете?", reply_markup=pbtn,
                                    parse_mode='html')
...
def otvet(message):
    if message.text == "Информация":
        sent_message = bot.send_message(message.chat.id, "Создатели: \n@nike_adidas_reebok\n@iuuuuuulyashka")
